  ...
  return (
    <RoleIntroContainer>
      <GapDiv>
        ...
        <RoleCondition>
          <RoleBox>
            <Text typo="H5">Editor 유지 조건</Text>
            <Condition>
              <ConditionItem text="누적 방문 일수 14일" />
              <ConditionItem text="개인기록 10개 이상 작성" />
              <ConditionItem text="커뮤니티 반응(좋아요, 댓글) 각 10개 이상" />
            </Condition>
          </RoleBox>
          <VectorIcon />
          <RoleBox>
            <Text typo="H5">등업 강등 조건</Text>
            <Condition>
              <ConditionItem text="내가 등록한 타일이 ‘부적절해요' 신고로 5회 이상 업로드 거부될 경우" />
              <ConditionItem text="한달 이상 미접속시, Watcher로 강등" />
            </Condition>
          </RoleBox>
        </RoleCondition>
      </GapDiv>
...
const ConditionItem = ({ text }: { text: string }) => (
  <FlexBox gap={8}>
    <CheckBlueIcon />
    <Text typo="Body_3" children={text} />
  </FlexBox>
);
