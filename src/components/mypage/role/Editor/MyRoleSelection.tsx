...
import { EditorToolTip } from "@/assets/images/role/EditorToolTip";
        ...
        <CurrentContainer>
          ...
          <AchieveDiv>
            <ProgressBarContainer>
              ...
              <EditorToolTipWrapper $rate={data.achievementRate}>
                <EditorToolTip achievementRate={data.achievementRate} />
              </EditorToolTipWrapper>
            </ProgressBarContainer>
            <Text typo="Caption_2" color="gray_1000">
              {`Editor 등급까지 ${data.achievementRate}% 달성했어요.`}
            </Text>
          </AchieveDiv>
        </CurrentContainer>
      </CurrentWrap>
...
const EditorToolTipWrapper = styled.div<{ $rate: number }>`
  position: absolute;
  top: -44px;
  left: ${({ $rate }) => `${$rate}%`};
